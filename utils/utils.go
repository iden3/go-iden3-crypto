...
// string.  It will return error if the string is not a decimal integer.
func NewIntFromString(s string) (*big.Int, error) {
	v, ok := new(big.Int).SetString(s, 10) //nolint:gomnd
	if !ok {
		return nil, fmt.Errorf("bad base 10 string %s", s)
	}
	return v, nil
}
